...
class HMM ():
    ...
    def forward(self, outcome, A, B):
        e2i = {e: i for i, e in enumerate(self.emits)}

        # build the start state
        # v grows down, it is organized as time x states
        v = np.zeros( (len(outcome), len(self.states)), dtype = 'longdouble')
        v[0] = B[e2i[outcome[0]]] / len(self.states)

        for i,c in enumerate(outcome[1:]):
            v[i+1] = v[i] @ A * B[e2i[c]]
    ...
    def backward(self, outcome, A, B):
        e2i = {e: i for i, e in enumerate(self.emits)}
        # build the end state
        v = np.zeros((len(outcome), len(self.states)), dtype='longdouble')

        v[-1] = np.ones( (1, len(self.states)), dtype = 'longdouble')
        # build the remainder of the viterbi graph

        for i,c in enumerate(reversed(outcome[1:])):
            v[-2-i] = v[-1-i] * B[e2i[c]] @ A.T

        return v

    def maximizePi(self, outcome):
        A = np.array([[self.transition[fr][to] for to in self.states] for fr in self.states], dtype='longdouble')
        B = np.array([[self.emission[st][e] for st in self.states] for e in self.emits], dtype='longdouble')
        e2i = {e: i for i, e in enumerate(self.emits)}
        s2i = {s: i for i, s in enumerate(self.states)}

        # build piStar
        leftViterbi = self.forward(outcome, A, B)
        rightViterbi = self.backward(outcome, A, B)

        piStar = rightViterbi * leftViterbi

        piStarStar = np.zeros( (len(outcome)-1, len(self.states), len(self.states)), dtype='longdouble')
        for i in range(len(outcome) - 1):
            piStarStar[i] = np.outer(leftViterbi[i], rightViterbi[i+1] * B[e2i[outcome[i + 1]]])
        newA = piStarStar.sum(axis=0) * A
        newA /= newA.sum(axis=1)  # normalize across the to direction

        arrOutcome = np.array(list(outcome))
        newB = np.zeros((len(self.emits),len(self.states)), dtype='longdouble')

        for i,c in enumerate(self.emits):
            newB[i] = sum(piStar[c==arrOutcome]) # emits x states
        newB /=  newB.sum(axis=0) # normalize across emits for each state

        self.emission = {state: {emit: newB[e2i[emit],s2i[state]] for emit in self.emits} for state in self.states}
        self.transition = {k: {l: newA[s2i[l], s2i[k]] for l in self.states} for k in self.states}
